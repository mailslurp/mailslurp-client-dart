//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:mailslurp/api.dart';
import 'package:test/test.dart';

// tests for WebhookResultDto
void main() {
  // final instance = WebhookResultDto();

  group('test WebhookResultDto', () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String userId
    test('to test the property `userId`', () async {
      // TODO
    });

    // String webhookId
    test('to test the property `webhookId`', () async {
      // TODO
    });

    // String webhookUrl
    test('to test the property `webhookUrl`', () async {
      // TODO
    });

    // String messageId
    test('to test the property `messageId`', () async {
      // TODO
    });

    // String redriveId
    test('to test the property `redriveId`', () async {
      // TODO
    });

    // String httpMethod
    test('to test the property `httpMethod`', () async {
      // TODO
    });

    // String webhookEvent
    test('to test the property `webhookEvent`', () async {
      // TODO
    });

    // int responseStatus
    test('to test the property `responseStatus`', () async {
      // TODO
    });

    // int responseTimeMillis
    test('to test the property `responseTimeMillis`', () async {
      // TODO
    });

    // String responseBodyExtract
    test('to test the property `responseBodyExtract`', () async {
      // TODO
    });

    // String resultType
    test('to test the property `resultType`', () async {
      // TODO
    });

    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // bool seen
    test('to test the property `seen`', () async {
      // TODO
    });

    // String inboxId
    test('to test the property `inboxId`', () async {
      // TODO
    });

    // String emailId
    test('to test the property `emailId`', () async {
      // TODO
    });

    // String attachmentId
    test('to test the property `attachmentId`', () async {
      // TODO
    });

    // String phoneId
    test('to test the property `phoneId`', () async {
      // TODO
    });

    // String smsId
    test('to test the property `smsId`', () async {
      // TODO
    });

    // String aiTransformerId
    test('to test the property `aiTransformerId`', () async {
      // TODO
    });


  });

}
