...
> DomainRegionGroupsDto getAvailableDomainRegions(inboxType)
...
try {
    final result = api_instance.getAvailableDomainRegions(inboxType);
    print(result);
} catch (e) {
    print('Exception when calling DomainControllerApi->getAvailableDomainRegions: $e\n');
}
```
