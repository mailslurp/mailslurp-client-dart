...
class SmsDto {
  ...
  @override
  String toString() => 'SmsDto[id=$id, userId=$userId, phoneNumber=$phoneNumber, fromNumber=$fromNumber, toNumber=$toNumber, favourite=$favourite, body=$body, read=$read, createdAt=$createdAt, updatedAt=$updatedAt]';
  ...
  static Map<String, List<SmsDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = SmsDto.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
