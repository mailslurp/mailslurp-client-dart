...
class ConnectorProjection {
  ...
  @override
  String toString() => 'ConnectorProjection[id=$id, userId=$userId, inboxId=$inboxId, createdAt=$createdAt, connectorType=$connectorType, syncScheduleType=$syncScheduleType, syncInterval=$syncInterval, syncEnabled=$syncEnabled]';
  ...
  static Map<String, List<ConnectorProjection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ConnectorProjection.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
