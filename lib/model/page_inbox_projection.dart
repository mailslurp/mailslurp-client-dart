...
class PageInboxProjection {
  ...
  @override
  String toString() => 'PageInboxProjection[content=$content, pageable=$pageable, total=$total, totalElements=$totalElements, totalPages=$totalPages, last=$last, numberOfElements=$numberOfElements, first=$first, size=$size, number=$number, sort=$sort, empty=$empty]';
  ...
  static Map<String, List<PageInboxProjection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = PageInboxProjection.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
