...
class PageInboxReplierDto {
  ...
  @override
  String toString() => 'PageInboxReplierDto[content=$content, pageable=$pageable, total=$total, totalElements=$totalElements, totalPages=$totalPages, last=$last, numberOfElements=$numberOfElements, first=$first, size=$size, number=$number, sort=$sort, empty=$empty]';
  ...
  static PageInboxReplierDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PageInboxReplierDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PageInboxReplierDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PageInboxReplierDto> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PageInboxReplierDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PageInboxReplierDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PageInboxReplierDto> mapFromJson(dynamic json) {
    final map = <String, PageInboxReplierDto>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PageInboxReplierDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<PageInboxReplierDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PageInboxReplierDto>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PageInboxReplierDto.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
