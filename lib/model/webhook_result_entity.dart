...
class WebhookResultEntity {
  ...
  WebhookResultEntityResultTypeEnum resultType;
  ...
  @override
  String toString() => 'WebhookResultEntity[createdAt=$createdAt, httpMethod=$httpMethod, id=$id, inboxId=$inboxId, messageId=$messageId, responseBodyExtract=$responseBodyExtract, responseStatus=$responseStatus, responseTimeMillis=$responseTimeMillis, resultType=$resultType, updatedAt=$updatedAt, userId=$userId, webhookEvent=$webhookEvent, webhookId=$webhookId, webhookUrl=$webhookUrl]';
  ...
  static WebhookResultEntity fromJson(Map<String, dynamic> json) => json == null
    ...
    : WebhookResultEntity(
        ...
        resultType: WebhookResultEntityResultTypeEnum.fromJson(json[r'resultType']),
...
class WebhookResultEntityResultTypeEnum {
  /// Instantiate a new enum with the provided [value].
  const WebhookResultEntityResultTypeEnum._(this.value);
  ...
  static const BAD_RESPONSE = WebhookResultEntityResultTypeEnum._(r'BAD_RESPONSE');
  static const EXCEPTION = WebhookResultEntityResultTypeEnum._(r'EXCEPTION');
  static const SUCCESS = WebhookResultEntityResultTypeEnum._(r'SUCCESS');

  /// List of all possible values in this [enum][WebhookResultEntityResultTypeEnum].
  static const values = <WebhookResultEntityResultTypeEnum>[
    BAD_RESPONSE,
    EXCEPTION,
    SUCCESS,
  ];

  static WebhookResultEntityResultTypeEnum fromJson(dynamic value) =>
    WebhookResultEntityResultTypeEnumTypeTransformer().decode(value);

  static List<WebhookResultEntityResultTypeEnum> listFromJson(List<dynamic> json, {bool emptyIsNull, bool growable,}) =>
    json == null || json.isEmpty
      ? true == emptyIsNull ? null : <WebhookResultEntityResultTypeEnum>[]
      : json
          .map((value) => WebhookResultEntityResultTypeEnum.fromJson(value))
          .toList(growable: true == growable);
}

/// Transformation class that can [encode] an instance of [WebhookResultEntityResultTypeEnum] to String,
/// and [decode] dynamic data back to [WebhookResultEntityResultTypeEnum].
class WebhookResultEntityResultTypeEnumTypeTransformer {
  const WebhookResultEntityResultTypeEnumTypeTransformer._();

  factory WebhookResultEntityResultTypeEnumTypeTransformer() => _instance ??= WebhookResultEntityResultTypeEnumTypeTransformer._();

  String encode(WebhookResultEntityResultTypeEnum data) => data.value;

  /// Decodes a [dynamic value][data] to a WebhookResultEntityResultTypeEnum.
  ///
  /// If [allowNull] is true and the [dynamic value][data] cannot be decoded successfully,
  /// then null is returned. However, if [allowNull] is false and the [dynamic value][data]
  /// cannot be decoded successfully, then an [UnimplementedError] is thrown.
  ...
  /// The [allowNull] is very handy when an API changes and a new enum value is added or removed,
  /// and users are still using an old app with the old code.
  WebhookResultEntityResultTypeEnum decode(dynamic data, {bool allowNull}) {
    switch (data) {
      case r'BAD_RESPONSE': return WebhookResultEntityResultTypeEnum.BAD_RESPONSE;
      case r'EXCEPTION': return WebhookResultEntityResultTypeEnum.EXCEPTION;
      case r'SUCCESS': return WebhookResultEntityResultTypeEnum.SUCCESS;
      default:
        if (allowNull == false) {
          throw ArgumentError('Unknown enum value to decode: $data');
        }
  ...
  /// Singleton [WebhookResultEntityResultTypeEnumTypeTransformer] instance.
  static WebhookResultEntityResultTypeEnumTypeTransformer _instance;
}
