...
class PhoneNumberProjection {
  ...
  @override
  String toString() => 'PhoneNumberProjection[id=$id, userId=$userId, createdAt=$createdAt, phoneNumber=$phoneNumber, phoneCountry=$phoneCountry]';
  ...
  static Map<String, List<PhoneNumberProjection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PhoneNumberProjection.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
