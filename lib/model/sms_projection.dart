...
class SmsProjection {
  ...
  @override
  String toString() => 'SmsProjection[userId=$userId, phoneNumber=$phoneNumber, fromNumber=$fromNumber, read=$read, body=$body, createdAt=$createdAt, id=$id]';
  ...
  static Map<String, List<SmsProjection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = SmsProjection.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
