...
class PageExpiredInboxRecordProjection {
  ...
  static PageExpiredInboxRecordProjection? fromJson(dynamic value) {
    if (value is Map) {
      ...
      return PageExpiredInboxRecordProjection(
        content: ExpiredInboxRecordProjection.listFromJson(json[r'content']) ?? const [],
  ...
  static Map<String, List<PageExpiredInboxRecordProjection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PageExpiredInboxRecordProjection.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
