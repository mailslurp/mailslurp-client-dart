...
class PageExpiredInboxRecordProjection {
  ...
  @override
  String toString() => 'PageExpiredInboxRecordProjection[content=$content, pageable=$pageable, total=$total, totalElements=$totalElements, totalPages=$totalPages, last=$last, numberOfElements=$numberOfElements, first=$first, size=$size, number=$number, sort=$sort, empty=$empty]';
  ...
  static PageExpiredInboxRecordProjection? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PageExpiredInboxRecordProjection[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PageExpiredInboxRecordProjection[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PageExpiredInboxRecordProjection> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PageExpiredInboxRecordProjection>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PageExpiredInboxRecordProjection.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PageExpiredInboxRecordProjection> mapFromJson(dynamic json) {
    final map = <String, PageExpiredInboxRecordProjection>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PageExpiredInboxRecordProjection.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<PageExpiredInboxRecordProjection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PageExpiredInboxRecordProjection>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PageExpiredInboxRecordProjection.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
