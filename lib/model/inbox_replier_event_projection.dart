...
class InboxReplierEventProjection {
  ...
  @override
  String toString() => 'InboxReplierEventProjection[message=$message, id=$id, status=$status, recipients=$recipients, userId=$userId, emailId=$emailId, inboxId=$inboxId, createdAt=$createdAt, sentId=$sentId, replierId=$replierId]';
  ...
  static Map<String, List<InboxReplierEventProjection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = InboxReplierEventProjection.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
