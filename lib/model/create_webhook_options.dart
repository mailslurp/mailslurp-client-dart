...
class CreateWebhookOptions {
  ...
  @override
  String toString() => 'CreateWebhookOptions[url=$url, basicAuth=$basicAuth, name=$name, eventName=$eventName, includeHeaders=$includeHeaders, requestBodyTemplate=$requestBodyTemplate, useStaticIpRange=$useStaticIpRange, ignoreInsecureSslCertificates=$ignoreInsecureSslCertificates]';
  ...
  static Map<String, List<CreateWebhookOptions>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateWebhookOptions.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
