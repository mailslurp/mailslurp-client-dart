...
class BouncedRecipientDto {
  ...
  @override
  String toString() => 'BouncedRecipientDto[id=$id, userId=$userId, sentEmailId=$sentEmailId, recipient=$recipient, diagnosticCode=$diagnosticCode, action=$action, bounceType=$bounceType, status=$status, createdAt=$createdAt]';
  ...
  static Map<String, List<BouncedRecipientDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = BouncedRecipientDto.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
