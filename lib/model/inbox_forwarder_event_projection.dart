...
class InboxForwarderEventProjection {
  ...
  @override
  String toString() => 'InboxForwarderEventProjection[message=$message, id=$id, status=$status, userId=$userId, emailId=$emailId, inboxId=$inboxId, createdAt=$createdAt, forwarderId=$forwarderId]';
  ...
  static Map<String, List<InboxForwarderEventProjection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = InboxForwarderEventProjection.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
