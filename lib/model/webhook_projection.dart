...
class WebhookProjection {
  ...
  @override
  String toString() => 'WebhookProjection[name=$name, id=$id, url=$url, inboxId=$inboxId, eventName=$eventName, createdAt=$createdAt, updatedAt=$updatedAt, phoneNumberId=$phoneNumberId]';
  ...
  static Map<String, List<WebhookProjection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = WebhookProjection.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
