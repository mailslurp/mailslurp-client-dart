//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

part of openapi.api;

class FlushExpiredInboxesResult {
  /// Returns a new [FlushExpiredInboxesResult] instance.
  FlushExpiredInboxesResult({
    this.inboxIds = const [],
    required this.expireBefore,
  });

  /// Inbox IDs affected by expiration
  List<String> inboxIds;

  /// DateTime to filter inboxes so that those expiring before this time are expired
  DateTime expireBefore;

  @override
  bool operator ==(Object other) => identical(this, other) || other is FlushExpiredInboxesResult &&
     other.inboxIds == inboxIds &&
     other.expireBefore == expireBefore;

  @override
  int get hashCode =>
    // ignore: unnecessary_parenthesis
    (inboxIds.hashCode) +
    (expireBefore.hashCode);

  @override
  String toString() => 'FlushExpiredInboxesResult[inboxIds=$inboxIds, expireBefore=$expireBefore]';

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
      json[r'inboxIds'] = this.inboxIds;
      json[r'expireBefore'] = this.expireBefore.toUtc().toIso8601String();
    return json;
  }

  /// Returns a new [FlushExpiredInboxesResult] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static FlushExpiredInboxesResult? fromJson(dynamic value) {
    if (value is Map) {
      final json = value.cast<String, dynamic>();

      // Ensure that the map contains the required keys.
      // Note 1: the values aren't checked for validity beyond being non-null.
      // Note 2: this code is stripped in release mode!
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FlushExpiredInboxesResult[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FlushExpiredInboxesResult[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return FlushExpiredInboxesResult(
        inboxIds: json[r'inboxIds'] is List
            ? (json[r'inboxIds'] as List).cast<String>()
            : const [],
        expireBefore: mapDateTime(json, r'expireBefore', '')!,
      );
    }
    return null;
  }

  static List<FlushExpiredInboxesResult> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FlushExpiredInboxesResult>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FlushExpiredInboxesResult.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
    }
    return result.toList(growable: growable);
  }

  static Map<String, FlushExpiredInboxesResult> mapFromJson(dynamic json) {
    final map = <String, FlushExpiredInboxesResult>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FlushExpiredInboxesResult.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
    }
    return map;
  }

  // maps a json object with a list of FlushExpiredInboxesResult-objects as value to a dart map
  static Map<String, List<FlushExpiredInboxesResult>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FlushExpiredInboxesResult>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FlushExpiredInboxesResult.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

  /// The list of required keys that must be present in a JSON.
  static const requiredKeys = <String>{
    'inboxIds',
    'expireBefore',
  };
}

