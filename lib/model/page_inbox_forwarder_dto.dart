...
class PageInboxForwarderDto {
  ...
  @override
  String toString() => 'PageInboxForwarderDto[content=$content, pageable=$pageable, total=$total, totalElements=$totalElements, totalPages=$totalPages, last=$last, numberOfElements=$numberOfElements, first=$first, size=$size, number=$number, sort=$sort, empty=$empty]';
  ...
  static Map<String, List<PageInboxForwarderDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = PageInboxForwarderDto.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
