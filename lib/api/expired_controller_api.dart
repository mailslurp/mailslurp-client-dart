...
class ExpiredControllerApi {
  ...
  Future<Response> getExpiredInboxesWithHttpInfo({ int? page, int? size, String? sort, DateTime? since, DateTime? before, String? inboxId, }) async {
    // ignore: prefer_const_declarations
  ...
  /// * [String] inboxId:
  ///   Optionally filter by inbox ID
  Future<PageExpiredInboxRecordProjection?> getExpiredInboxes({ int? page, int? size, String? sort, DateTime? since, DateTime? before, String? inboxId, }) async {
    final response = await getExpiredInboxesWithHttpInfo( page: page, size: size, sort: sort, since: since, before: before, inboxId: inboxId, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
