...
class MailServerControllerApi {
  ...
  /// Parameters:
  ///
  /// * [DNSLookupsOptions] dNSLookupsOptions (required):
  Future<Response> getDnsLookupsWithHttpInfo(DNSLookupsOptions dNSLookupsOptions,) async {
    // ignore: prefer_const_declarations
    final path = r'/mail-server/describe/dns-lookups';

    // ignore: prefer_final_locals
    Object? postBody = dNSLookupsOptions;
  ...
  ///
  /// * [DNSLookupsOptions] dNSLookupsOptions (required):
  Future<DNSLookupResults?> getDnsLookups(DNSLookupsOptions dNSLookupsOptions,) async {
    final response = await getDnsLookupsWithHttpInfo(dNSLookupsOptions,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
