...
class TemplateControllerApi {
  ...
  Future<Response> getAllTemplatesWithHttpInfo({ DateTime before, int page, DateTime since, int size, String sort }) async {
    // Verify required params are set.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageTemplateProjection> getAllTemplates({ DateTime before, int page, DateTime since, int size, String sort }) async {
    final response = await getAllTemplatesWithHttpInfo( before: before, page: page, since: since, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
