...
class ImapControllerApi {
  ...
  /// * [String] inboxId:
  ///   Inbox ID to search
  Future<Response> imapServerGetWithHttpInfo(String emailId, { String? inboxId, }) async {
    // ignore: prefer_const_declarations
    final path = r'/imap/server/get';
  ...
  /// * [String] inboxId:
  ///   Inbox ID to search
  Future<ImapServerGetResult?> imapServerGet(String emailId, { String? inboxId, }) async {
    final response = await imapServerGetWithHttpInfo(emailId,  inboxId: inboxId, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ImapServerGetResult',) as ImapServerGetResult;
  ...
  Future<Response> imapServerMailboxWithHttpInfo(String name,) async {
    // ignore: prefer_const_declarations
    final path = r'/imap/server/mailbox';
  ...
  /// * [String] name (required):
  ///   Inbox email address to create
  Future<ImapServerMailboxResult?> imapServerMailbox(String name,) async {
    final response = await imapServerMailboxWithHttpInfo(name,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ImapServerMailboxResult',) as ImapServerMailboxResult;
