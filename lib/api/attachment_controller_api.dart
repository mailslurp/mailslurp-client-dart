...
class AttachmentControllerApi {
  ...
  /// * [String] attachmentId (required):
  ///   ID of attachment
  Future<List<String>> downloadAttachmentAsBytes(String attachmentId) async {
  ...
  Future<Response> getAttachments1WithHttpInfo({ int page, int size, String sort, String fileNameFilter, DateTime since, DateTime before }) async {
    // Verify required params are set.
  ...
  /// * [DateTime] before:
  ///   Filter by created at before the given timestamp
  Future<PageAttachmentEntity> getAttachments1({ int page, int size, String sort, String fileNameFilter, DateTime since, DateTime before }) async {
    final response = await getAttachments1WithHttpInfo( page: page, size: size, sort: sort, fileNameFilter: fileNameFilter, since: since, before: before );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
