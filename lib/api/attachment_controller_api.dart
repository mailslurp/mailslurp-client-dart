...
class AttachmentControllerApi {
  ...
  Future<Response> uploadAttachmentBytesWithHttpInfo(InlineObject1 inlineObject1, { String contentType, String filename }) async {
    // Verify required params are set.
  ...
  Future<List<String>> uploadAttachmentBytes(InlineObject1 inlineObject1, { String contentType, String filename }) async {
    final response = await uploadAttachmentBytesWithHttpInfo(inlineObject1,  contentType: contentType, filename: filename );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
