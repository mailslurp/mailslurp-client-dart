...
class AttachmentControllerApi {
  ...
  /// Upload an attachment for sending using base64 file encoding with HTTP info returned
  ///
  ...
  /// Upload an attachment for sending using file byte stream input octet stream with HTTP info returned
  ///
  /// When sending emails with attachments first upload each attachment with this endpoint. Record the returned attachment IDs. Then use these attachment IDs in the SendEmailOptions when sending an email. This means that attachments can easily be reused.
  Future<Response> uploadAttachmentBytesWithHttpInfo({ String filename }) async {
    Object postBody = filename;
  ...
  ///
  /// When sending emails with attachments first upload each attachment with this endpoint. Record the returned attachment IDs. Then use these attachment IDs in the SendEmailOptions when sending an email. This means that attachments can easily be reused.
  Future<List<String>> uploadAttachmentBytes({ String filename }) async {
    Response response = await uploadAttachmentBytesWithHttpInfo( filename: filename );
    if(response.statusCode >= 400) {
      throw ApiException(response.statusCode, _decodeBodyBytes(response));
    } else if(response.body != null) {
      return (apiClient.deserialize(_decodeBodyBytes(response), 'List<String>') as List).map((item) => item as String).toList();
