...
class AttachmentControllerApi {
  ...
  Future<Response> getAttachmentsWithHttpInfo({ int? page, int? size, String? sort, String? fileNameFilter, DateTime? since, DateTime? before, String? inboxId, }) async {
    // ignore: prefer_const_declarations
    final path = r'/attachments';
  ...
  /// * [String] inboxId:
  ///   Optional inboxId to filter attachments by
  Future<PageAttachmentEntity?> getAttachments({ int? page, int? size, String? sort, String? fileNameFilter, DateTime? since, DateTime? before, String? inboxId, }) async {
    final response = await getAttachmentsWithHttpInfo( page: page, size: size, sort: sort, fileNameFilter: fileNameFilter, since: since, before: before, inboxId: inboxId, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
