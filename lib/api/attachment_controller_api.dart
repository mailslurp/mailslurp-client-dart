...
class AttachmentControllerApi {
  ...
  Future<Response> getAttachmentsWithHttpInfo({ DateTime before, String fileNameFilter, int page, DateTime since, int size, String sort }) async {
    // Verify required params are set.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageAttachmentEntity> getAttachments({ DateTime before, String fileNameFilter, int page, DateTime since, int size, String sort }) async {
    final response = await getAttachmentsWithHttpInfo( before: before, fileNameFilter: fileNameFilter, page: page, since: since, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> uploadAttachmentBytesWithHttpInfo({ String contentType, String filename, String byteArray }) async {
    // Verify required params are set.
  ...
  /// * [String] byteArray:
  ///   Byte array request body
  Future<List<String>> uploadAttachmentBytes({ String contentType, String filename, String byteArray }) async {
    final response = await uploadAttachmentBytesWithHttpInfo( contentType: contentType, filename: filename, byteArray: byteArray );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
