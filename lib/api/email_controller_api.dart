...
class EmailControllerApi {
  ...
  Future<Response> searchEmailsWithHttpInfo(SearchEmailsOptions searchEmailsOptions,) async {
    // ignore: prefer_const_declarations
  ...
  Future<PageEmailProjection?> searchEmails(SearchEmailsOptions searchEmailsOptions,) async {
    final response = await searchEmailsWithHttpInfo(searchEmailsOptions,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
