...
class EmailControllerApi {
  ...
  Future<Response> getEmailHTMLQueryWithHttpInfo(String emailId, String htmlSelector) async {
    ...
    if(htmlSelector == null) {
     throw ApiException(400, "Missing required param: htmlSelector");
    }

    // create path and map variables
    String path = "/emails/{emailId}/html-query".replaceAll("{format}","json").replaceAll("{" + "emailId" + "}", emailId.toString()).replaceAll("{" + "htmlSelector" + "}", htmlSelector.toString());
  ...
  Future<EmailTextLinesResult> getEmailHTMLQuery(String emailId, String htmlSelector) async {
    Response response = await getEmailHTMLQueryWithHttpInfo(emailId, htmlSelector);
    if(response.statusCode >= 400) {
      throw ApiException(response.statusCode, _decodeBodyBytes(response));
    } else if(response.body != null) {
  ...
  Future<Response> getEmailTextLinesWithHttpInfo(bool decodeHtmlEntities, String emailId) async {
    Object postBody;

    // verify required params are set
    if(decodeHtmlEntities == null) {
     throw ApiException(400, "Missing required param: decodeHtmlEntities");
    }
    if(emailId == null) {
     throw ApiException(400, "Missing required param: emailId");
    }
    ...
    // create path and map variables
    String path = "/emails/{emailId}/text".replaceAll("{format}","json").replaceAll("{" + "decodeHtmlEntities" + "}", decodeHtmlEntities.toString()).replaceAll("{" + "emailId" + "}", emailId.toString());
  ...
  ///
  /// Parse an email body and return the content as an array of strings. HTML parsing uses JSoup and UNIX line separators.
  Future<EmailTextLinesResult> getEmailTextLines(bool decodeHtmlEntities, String emailId) async {
    Response response = await getEmailTextLinesWithHttpInfo(decodeHtmlEntities, emailId);
    if(response.statusCode >= 400) {
      throw ApiException(response.statusCode, _decodeBodyBytes(response));
    } else if(response.body != null) {
      return apiClient.deserialize(_decodeBodyBytes(response), 'EmailTextLinesResult') as EmailTextLinesResult;
