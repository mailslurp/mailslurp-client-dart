...
class EmailControllerApi {
  ...
  Future<Response> getEmailsOffsetPaginatedWithHttpInfo({ List<String>? inboxId, int? page, int? size, String? sort, bool? unreadOnly, String? searchFilter, DateTime? since, DateTime? before, bool? favourited, bool? syncConnectors, String? plusAddressId, }) async {
    // ignore: prefer_const_declarations
  ...
  /// * [String] plusAddressId:
  ///   Optional plus address ID filter
  Future<PageEmailProjection?> getEmailsOffsetPaginated({ List<String>? inboxId, int? page, int? size, String? sort, bool? unreadOnly, String? searchFilter, DateTime? since, DateTime? before, bool? favourited, bool? syncConnectors, String? plusAddressId, }) async {
    final response = await getEmailsOffsetPaginatedWithHttpInfo( inboxId: inboxId, page: page, size: size, sort: sort, unreadOnly: unreadOnly, searchFilter: searchFilter, since: since, before: before, favourited: favourited, syncConnectors: syncConnectors, plusAddressId: plusAddressId, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
