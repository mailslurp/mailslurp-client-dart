...
class EmailControllerApi {
  ...
  /// * [String] apiKey:
  ///   Can pass apiKey in url for this request if you wish to download the file in a browser. Content type will be set to original content type of the attachment file. This is so that browsers can download the file correctly.
  Future<List<String>> downloadAttachment(String emailId, String attachmentId, { String apiKey }) async {
  ...
  /// * [String] emailId (required):
  ///   ID of email
  Future<List<String>> downloadBodyBytes(String emailId) async {
  ...
  Future<Response> getLatestEmailWithHttpInfo({ Set<String> inboxIds }) async {
    // Verify required params are set.
  ...
  /// * [Set<String>] inboxIds:
  ///   Optional set of inboxes to filter by. Only get the latest email from these inbox IDs. If not provided will search across all inboxes
  Future<Email> getLatestEmail({ Set<String> inboxIds }) async {
  ...
  Future<Email> getLatestEmailInInbox(String inboxId) async {
    final response = await getLatestEmailInInboxWithHttpInfo(inboxId);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> sendEmailSourceOptionalWithHttpInfo(SendEmailOptions sendEmailOptions, { String inboxId, bool useDomainPool }) async {
    // Verify required params are set.
  ...
  /// * [bool] useDomainPool:
  ///   Use domain pool. Optionally create inbox to send from using the mailslurp domain pool.
  Future<void> sendEmailSourceOptional(SendEmailOptions sendEmailOptions, { String inboxId, bool useDomainPool }) async {
    final response = await sendEmailSourceOptionalWithHttpInfo(sendEmailOptions,  inboxId: inboxId, useDomainPool: useDomainPool );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
