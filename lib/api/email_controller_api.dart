...
class EmailControllerApi {
  ...
  Future<Response> sendEmailSourceOptionalWithHttpInfo(String inboxId, SendEmailOptions sendEmailOptions, { bool useDomainPool }) async {
    // Verify required params are set.
    if (inboxId == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: inboxId');
    }
  ...
  /// * [bool] useDomainPool:
  ///   Use domain pool. Optionally create inbox to send from using the mailslurp domain pool.
  Future<void> sendEmailSourceOptional(String inboxId, SendEmailOptions sendEmailOptions, { bool useDomainPool }) async {
    final response = await sendEmailSourceOptionalWithHttpInfo(inboxId, sendEmailOptions,  useDomainPool: useDomainPool );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
