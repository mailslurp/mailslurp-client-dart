...
class EmailControllerApi {
  ...
  Future<Response> getEmailsPaginatedWithHttpInfo({ List<String> inboxId, int page, String searchFilter, int size, String sort, bool unreadOnly }) async {
    // Verify required params are set.
  ...
  /// * [bool] unreadOnly:
  ///   Optional filter for unread emails only. All emails are considered unread until they are viewed in the dashboard or requested directly
  Future<PageEmailProjection> getEmailsPaginated({ List<String> inboxId, int page, String searchFilter, int size, String sort, bool unreadOnly }) async {
    final response = await getEmailsPaginatedWithHttpInfo( inboxId: inboxId, page: page, searchFilter: searchFilter, size: size, sort: sort, unreadOnly: unreadOnly );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  /// * [bool] unreadOnly:
  ///   Optional filter for unread emails only. All emails are considered unread until they are viewed in the dashboard or requested directly
  Future<Response> getOrganizationEmailsPaginatedWithHttpInfo({ List<String> inboxId, int page, String searchFilter, int size, String sort, bool unreadOnly }) async {
    // Verify required params are set.
  ...
  /// * [bool] unreadOnly:
  ///   Optional filter for unread emails only. All emails are considered unread until they are viewed in the dashboard or requested directly
  Future<PageEmailProjection> getOrganizationEmailsPaginated({ List<String> inboxId, int page, String searchFilter, int size, String sort, bool unreadOnly }) async {
    final response = await getOrganizationEmailsPaginatedWithHttpInfo( inboxId: inboxId, page: page, searchFilter: searchFilter, size: size, sort: sort, unreadOnly: unreadOnly );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
