...
class EmailControllerApi {
  ...
  /// * [String] emailId (required):
  ///   ID of email
  Future<Response> getAttachments1WithHttpInfo(String emailId) async {
    // Verify required params are set.
    if (emailId == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: emailId');
    }
  ...
  /// * [String] emailId (required):
  ///   ID of email
  Future<List<AttachmentMetaData>> getAttachments1(String emailId) async {
    final response = await getAttachments1WithHttpInfo(emailId);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> getEmailsPaginatedWithHttpInfo({ DateTime before, List<String> inboxId, int page, String searchFilter, DateTime since, int size, String sort, bool unreadOnly }) async {
    // Verify required params are set.
  ...
  /// * [bool] unreadOnly:
  ///   Optional filter for unread emails only. All emails are considered unread until they are viewed in the dashboard or requested directly
  Future<PageEmailProjection> getEmailsPaginated({ DateTime before, List<String> inboxId, int page, String searchFilter, DateTime since, int size, String sort, bool unreadOnly }) async {
    final response = await getEmailsPaginatedWithHttpInfo( before: before, inboxId: inboxId, page: page, searchFilter: searchFilter, since: since, size: size, sort: sort, unreadOnly: unreadOnly );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  /// * [bool] unreadOnly:
  ///   Optional filter for unread emails only. All emails are considered unread until they are viewed in the dashboard or requested directly
  Future<Response> getOrganizationEmailsPaginatedWithHttpInfo({ DateTime before, List<String> inboxId, int page, String searchFilter, DateTime since, int size, String sort, bool unreadOnly }) async {
    // Verify required params are set.
  ...
  /// * [bool] unreadOnly:
  ///   Optional filter for unread emails only. All emails are considered unread until they are viewed in the dashboard or requested directly
  Future<PageEmailProjection> getOrganizationEmailsPaginated({ DateTime before, List<String> inboxId, int page, String searchFilter, DateTime since, int size, String sort, bool unreadOnly }) async {
    final response = await getOrganizationEmailsPaginatedWithHttpInfo( before: before, inboxId: inboxId, page: page, searchFilter: searchFilter, since: since, size: size, sort: sort, unreadOnly: unreadOnly );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> sendEmailSourceOptionalWithHttpInfo({ String inboxId, bool useDomainPool, SendEmailOptions sendEmailOptions }) async {
    // Verify required params are set.
  ...
  /// * [SendEmailOptions] sendEmailOptions:
  ///   Options for the email
  Future<void> sendEmailSourceOptional({ String inboxId, bool useDomainPool, SendEmailOptions sendEmailOptions }) async {
    final response = await sendEmailSourceOptionalWithHttpInfo( inboxId: inboxId, useDomainPool: useDomainPool, sendEmailOptions: sendEmailOptions );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
