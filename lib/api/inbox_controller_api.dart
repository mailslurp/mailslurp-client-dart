...
class InboxControllerApi {
  ...
  Future<Response> createInboxWithHttpInfo({ String emailAddress, List<String> tags, String name, String description, bool useDomainPool, bool favourite, DateTime expiresAt, int expiresIn, bool allowTeamAccess, String inboxType }) async {
  ...
  Future<InboxDto> createInbox({ String emailAddress, List<String> tags, String name, String description, bool useDomainPool, bool favourite, DateTime expiresAt, int expiresIn, bool allowTeamAccess, String inboxType }) async {
    final response = await createInboxWithHttpInfo( emailAddress: emailAddress, tags: tags, name: name, description: description, useDomainPool: useDomainPool, favourite: favourite, expiresAt: expiresAt, expiresIn: expiresIn, allowTeamAccess: allowTeamAccess, inboxType: inboxType );
  ...
  ///
  /// * [int] aliasForLimitPeriodAssessedFirstBeforeAssessingAnyPassedLimitPeriod:
  ///
  /// * [int] limit:
  ...
  Future<Response> getEmailsWithHttpInfo(String inboxId, { int aliasForLimitPeriodAssessedFirstBeforeAssessingAnyPassedLimitPeriod, int limit, String sortTheResultsByReceivedDateAndDirectionASCOrDESC, int retryTimeout, int delayTimeout, int minCount, bool unreadOnly, DateTime before, DateTime since }) async {
    // Verify required params are set.
    if (inboxId == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: inboxId');
    }
    ...
    if (aliasForLimitPeriodAssessedFirstBeforeAssessingAnyPassedLimitPeriod != null) {
      queryParams.addAll(_convertParametersForCollectionFormat('', 'Alias for limit. Assessed first before assessing any passed limit.', aliasForLimitPeriodAssessedFirstBeforeAssessingAnyPassedLimitPeriod));
    }
    if (limit != null) {
      queryParams.addAll(_convertParametersForCollectionFormat('', 'limit', limit));
    }
    if (sortTheResultsByReceivedDateAndDirectionASCOrDESC != null) {
      queryParams.addAll(_convertParametersForCollectionFormat('', 'Sort the results by received date and direction ASC or DESC', sortTheResultsByReceivedDateAndDirectionASCOrDESC));
    }
  ...
  /// * [DateTime] since:
  ///   Exclude emails received before this ISO 8601 date time
  Future<List<EmailPreview>> getEmails(String inboxId, { int aliasForLimitPeriodAssessedFirstBeforeAssessingAnyPassedLimitPeriod, int limit, String sortTheResultsByReceivedDateAndDirectionASCOrDESC, int retryTimeout, int delayTimeout, int minCount, bool unreadOnly, DateTime before, DateTime since }) async {
    final response = await getEmailsWithHttpInfo(inboxId,  aliasForLimitPeriodAssessedFirstBeforeAssessingAnyPassedLimitPeriod: aliasForLimitPeriodAssessedFirstBeforeAssessingAnyPassedLimitPeriod, limit: limit, sortTheResultsByReceivedDateAndDirectionASCOrDESC: sortTheResultsByReceivedDateAndDirectionASCOrDESC, retryTimeout: retryTimeout, delayTimeout: delayTimeout, minCount: minCount, unreadOnly: unreadOnly, before: before, since: since );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Set<String>> getInboxTags() async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body != null && response.statusCode != HttpStatus.noContent) {
      return (await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'Set<String>') as List)
        .cast<String>()
        .toSet();
    }
    return Future<Set<String>>.value(null);
  }
