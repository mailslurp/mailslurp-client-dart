...
class InboxRulesetControllerApi {
  ...
  Future<Response> getInboxRulesetsWithHttpInfo({ String inboxId, int page, String searchFilter, int size, String sort }) async {
    // Verify required params are set.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageInboxRulesetDto> getInboxRulesets({ String inboxId, int page, String searchFilter, int size, String sort }) async {
    final response = await getInboxRulesetsWithHttpInfo( inboxId: inboxId, page: page, searchFilter: searchFilter, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
