...
class InboxRulesetControllerApi {
  ...
  /// Parameters:
  ///
  /// * [TestInboxRulesetReceivingOptions] testInboxRulesetReceivingOptions (required):
  Future<Response> testInboxRulesetReceivingWithHttpInfo(TestInboxRulesetReceivingOptions testInboxRulesetReceivingOptions,) async {
    // ignore: prefer_const_declarations
    final path = r'/rulesets/test-receiving';

    // ignore: prefer_final_locals
    Object? postBody = testInboxRulesetReceivingOptions;
  ...
  ///
  /// * [TestInboxRulesetReceivingOptions] testInboxRulesetReceivingOptions (required):
  Future<TestInboxRulesetReceivingResult?> testInboxRulesetReceiving(TestInboxRulesetReceivingOptions testInboxRulesetReceivingOptions,) async {
    final response = await testInboxRulesetReceivingWithHttpInfo(testInboxRulesetReceivingOptions,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'TestInboxRulesetReceivingResult',) as TestInboxRulesetReceivingResult;
  ...
  Future<Response> testInboxRulesetSendingWithHttpInfo(TestInboxRulesetSendingOptions testInboxRulesetSendingOptions,) async {
    // ignore: prefer_const_declarations
  ...
  ///
  /// * [TestInboxRulesetSendingOptions] testInboxRulesetSendingOptions (required):
  Future<TestInboxRulesetSendingResult?> testInboxRulesetSending(TestInboxRulesetSendingOptions testInboxRulesetSendingOptions,) async {
    final response = await testInboxRulesetSendingWithHttpInfo(testInboxRulesetSendingOptions,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'TestInboxRulesetSendingResult',) as TestInboxRulesetSendingResult;
