...
class InboxRulesetControllerApi {
  ...
  Future<Response> getInboxRulesetsWithHttpInfo({ DateTime before, String inboxId, int page, String searchFilter, DateTime since, int size, String sort }) async {
    // Verify required params are set.
  ...
  Future<PageInboxRulesetDto> getInboxRulesets({ DateTime before, String inboxId, int page, String searchFilter, DateTime since, int size, String sort }) async {
    final response = await getInboxRulesetsWithHttpInfo( before: before, inboxId: inboxId, page: page, searchFilter: searchFilter, since: since, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
