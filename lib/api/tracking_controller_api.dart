...
class TrackingControllerApi {
  ...
  Future<Response> getAllTrackingPixelsWithHttpInfo({ int page, int size, String sort }) async {
    // Verify required params are set.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageTrackingPixelProjection> getAllTrackingPixels({ int page, int size, String sort }) async {
    final response = await getAllTrackingPixelsWithHttpInfo( page: page, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
