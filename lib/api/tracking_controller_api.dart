...
class TrackingControllerApi {
  ...
  Future<Response> getAllTrackingPixelsWithHttpInfo({ DateTime before, int page, String searchFilter, DateTime since, int size, String sort }) async {
    // Verify required params are set.
  ...
  Future<PageTrackingPixelProjection> getAllTrackingPixels({ DateTime before, int page, String searchFilter, DateTime since, int size, String sort }) async {
    final response = await getAllTrackingPixelsWithHttpInfo( before: before, page: page, searchFilter: searchFilter, since: since, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
