...
class MissedEmailControllerApi {
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<Response> getAllUnknownMissedEmailsWithHttpInfo({ String inboxId, int page, String searchFilter, int size, String sort }) async {
    // Verify required params are set.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageUnknownMissedEmailProjection> getAllUnknownMissedEmails({ String inboxId, int page, String searchFilter, int size, String sort }) async {
    final response = await getAllUnknownMissedEmailsWithHttpInfo( inboxId: inboxId, page: page, searchFilter: searchFilter, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
