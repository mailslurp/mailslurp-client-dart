...
class MissedEmailControllerApi {
  ...
  Future<Response> waitForNthMissedEmailWithHttpInfo(String inboxId, int timeout, { int index }) async {
    // Verify required params are set.
    if (inboxId == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: inboxId');
    }
    if (timeout == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: timeout');
    }
  ...
  /// * [int] index:
  ///   Zero based index of the email to wait for. If 1 missed email already and you want to wait for the 2nd email pass index=1
  Future<MissedEmail> waitForNthMissedEmail(String inboxId, int timeout, { int index }) async {
    final response = await waitForNthMissedEmailWithHttpInfo(inboxId, timeout,  index: index );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
