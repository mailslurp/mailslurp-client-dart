...
class BounceControllerApi {
  ...
  Future<Response> getBouncedEmailsWithHttpInfo({ DateTime before, int page, DateTime since, int size, String sort }) async {
    // Verify required params are set.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageBouncedEmail> getBouncedEmails({ DateTime before, int page, DateTime since, int size, String sort }) async {
    final response = await getBouncedEmailsWithHttpInfo( before: before, page: page, since: since, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<Response> getBouncedRecipientsWithHttpInfo({ DateTime before, int page, DateTime since, int size, String sort }) async {
    // Verify required params are set.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageBouncedRecipients> getBouncedRecipients({ DateTime before, int page, DateTime since, int size, String sort }) async {
    final response = await getBouncedRecipientsWithHttpInfo( before: before, page: page, since: since, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
