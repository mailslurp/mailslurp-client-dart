...
class BounceControllerApi {
  ...
  /// * [DateTime] before:
  ///   Filter by created at before the given timestamp
  Future<Response> getReputationItemsWithHttpInfo({ int? page, int? size, String? sort, DateTime? since, DateTime? before, }) async {
    // ignore: prefer_const_declarations
    final path = r'/bounce/reputation-items';
  ...
  Future<PageReputationItems?> getReputationItems({ int? page, int? size, String? sort, DateTime? since, DateTime? before, }) async {
    final response = await getReputationItemsWithHttpInfo( page: page, size: size, sort: sort, since: since, before: before, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'PageReputationItems',) as PageReputationItems;
  ...
  ///   Optional account region filter
  Future<Response> getTenantReputationFindingsWithHttpInfo({ String? accountRegion, }) async {
    // ignore: prefer_const_declarations
    final path = r'/bounce/tenant-findings';
    ...
    if (accountRegion != null) {
      queryParams.addAll(_queryParams('', 'accountRegion', accountRegion));
    }
  ...
  /// * [String] accountRegion:
  ///   Optional account region filter
  Future<TenantReputationFindingsDto?> getTenantReputationFindings({ String? accountRegion, }) async {
    final response = await getTenantReputationFindingsWithHttpInfo( accountRegion: accountRegion, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'TenantReputationFindingsDto',) as TenantReputationFindingsDto;
  ...
  /// * [String] accountRegion:
  ///   Optional account region filter
  Future<Response> getTenantReputationStatusSummaryWithHttpInfo({ String? accountRegion, }) async {
    // ignore: prefer_const_declarations
    final path = r'/bounce/tenant-status';
  ...
  /// * [String] accountRegion:
  ///   Optional account region filter
  Future<TenantReputationStatusSummaryDto?> getTenantReputationStatusSummary({ String? accountRegion, }) async {
    final response = await getTenantReputationStatusSummaryWithHttpInfo( accountRegion: accountRegion, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'TenantReputationStatusSummaryDto',) as TenantReputationStatusSummaryDto;
