...
class CommonActionsControllerApi {
  ...
  Future<Inbox> createNewEmailAddress({ bool allowTeamAccess, DateTime expiresAt, int expiresIn, bool useDomainPool }) async {
    final response = await createNewEmailAddressWithHttpInfo( allowTeamAccess: allowTeamAccess, expiresAt: expiresAt, expiresIn: expiresIn, useDomainPool: useDomainPool );
  ...
  Future<Inbox> createNewEmailAddress1({ bool allowTeamAccess, DateTime expiresAt, int expiresIn, bool useDomainPool }) async {
    final response = await createNewEmailAddress1WithHttpInfo( allowTeamAccess: allowTeamAccess, expiresAt: expiresAt, expiresIn: expiresIn, useDomainPool: useDomainPool );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
