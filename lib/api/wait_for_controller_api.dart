...
class WaitForControllerApi {
  ...
  Future<Response> waitForLatestSmsWithHttpInfo(WaitForSingleSmsOptions waitForSingleSmsOptions) async {
    ...
    if (waitForSingleSmsOptions == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: waitForSingleSmsOptions');
    }
  ...
  Future<Response> waitForSmsWithHttpInfo(WaitForSmsConditions waitForSmsConditions) async {
    ...
    if (waitForSmsConditions == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: waitForSmsConditions');
    }
  ...
  Future<List<SmsPreview>> waitForSms(WaitForSmsConditions waitForSmsConditions) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body != null && response.statusCode != HttpStatus.noContent) {
      return (await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'List<SmsPreview>') as List)
        .cast<SmsPreview>()
        .toList(growable: false);
    }
    return Future<List<SmsPreview>>.value(null);
  }
}
