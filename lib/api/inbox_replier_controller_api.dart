...
class InboxReplierControllerApi {
  ...
  Future<Response> getAllInboxReplierEventsWithHttpInfo({ String? inboxReplierId, String? inboxId, String? emailId, String? sentId, int? page, int? size, String? sort, }) async {
    // ignore: prefer_const_declarations
    final path = r'/repliers/events';
    ...
    if (inboxReplierId != null) {
      queryParams.addAll(_queryParams('', 'inboxReplierId', inboxReplierId));
    }
    if (inboxId != null) {
      queryParams.addAll(_queryParams('', 'inboxId', inboxId));
    }
  ...
  Future<PageInboxReplierEvents?> getAllInboxReplierEvents({ String? inboxReplierId, String? inboxId, String? emailId, String? sentId, int? page, int? size, String? sort, }) async {
    final response = await getAllInboxReplierEventsWithHttpInfo( inboxReplierId: inboxReplierId, inboxId: inboxId, emailId: emailId, sentId: sentId, page: page, size: size, sort: sort, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
