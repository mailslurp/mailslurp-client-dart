...
class WebhookControllerApi {
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getTestWebhookPayloadWithHttpInfo() async {
  ...
  /// Get test webhook payload example
  Future<WebhookPayload> getTestWebhookPayload() async {
    final response = await getTestWebhookPayloadWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body != null && response.statusCode != HttpStatus.noContent) {
      return apiClient.deserialize(_decodeBodyBytes(response), 'WebhookPayload') as WebhookPayload;
        }
    return Future<WebhookPayload>.value(null);
  }
