...
class GroupControllerApi {
  ...
  Future<Response> getAllGroupsWithHttpInfo({ DateTime before, int page, DateTime since, int size, String sort }) async {
    // Verify required params are set.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageGroupProjection> getAllGroups({ DateTime before, int page, DateTime since, int size, String sort }) async {
    final response = await getAllGroupsWithHttpInfo( before: before, page: page, since: since, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<Response> getGroupWithContactsPaginatedWithHttpInfo(String groupId, { DateTime before, int page, DateTime since, int size, String sort }) async {
    // Verify required params are set.
    if (groupId == null) {
     throw ApiException(HttpStatus.badRequest, 'Missing required param: groupId');
    }
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageContactProjection> getGroupWithContactsPaginated(String groupId, { DateTime before, int page, DateTime since, int size, String sort }) async {
    final response = await getGroupWithContactsPaginatedWithHttpInfo(groupId,  before: before, page: page, since: since, size: size, sort: sort );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
