...
class SmsControllerApi {
  ...
  ///
  /// * [String] smsId (required):
  ///   ID of SMS to extract codes from
  ///
  /// * [ExtractCodesOptions] extractCodesOptions:
  Future<Response> getSmsCodesWithHttpInfo(String smsId, { ExtractCodesOptions? extractCodesOptions, }) async {
    // ignore: prefer_const_declarations
    final path = r'/sms/{smsId}/codes'
      .replaceAll('{smsId}', smsId);
  ...
  ///
  /// * [ExtractCodesOptions] extractCodesOptions:
  Future<ExtractCodesResult?> getSmsCodes(String smsId, { ExtractCodesOptions? extractCodesOptions, }) async {
    final response = await getSmsCodesWithHttpInfo(smsId,  extractCodesOptions: extractCodesOptions, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// * [String] phoneNumberId:
  Future<Response> markAllSmsAsReadWithHttpInfo({ bool? read, String? phoneNumberId, }) async {
    // ignore: prefer_const_declarations
    final path = r'/sms/read';
  ...
  ///
  /// * [String] phoneNumberId:
  Future<void> markAllSmsAsRead({ bool? read, String? phoneNumberId, }) async {
    final response = await markAllSmsAsReadWithHttpInfo( read: read, phoneNumberId: phoneNumberId, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
  ...
  ///
  /// * [bool] read:
  Future<Response> markSmsAsReadWithHttpInfo(String smsId, { bool? read, }) async {
    // ignore: prefer_const_declarations
    final path = r'/sms/{smsId}/read'
      .replaceAll('{smsId}', smsId);
  ...
  ///
  /// * [bool] read:
  Future<SmsDto?> markSmsAsRead(String smsId, { bool? read, }) async {
    final response = await markSmsAsReadWithHttpInfo(smsId,  read: read, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
