...
class DomainControllerApi {
  ...
  ///
  /// * [String] inboxType:
  Future<Response> getAvailableDomainRegionsWithHttpInfo({ String? inboxType, }) async {
    // ignore: prefer_const_declarations
    final path = r'/domains/available-domain-regions';
  ...
  ///
  /// * [String] inboxType:
  Future<DomainRegionGroupsDto?> getAvailableDomainRegions({ String? inboxType, }) async {
    final response = await getAvailableDomainRegionsWithHttpInfo( inboxType: inboxType, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'DomainRegionGroupsDto',) as DomainRegionGroupsDto;
  ...
  ///
  /// * [String] inboxType:
  Future<Response> getAvailableDomainsWithHttpInfo({ String? inboxType, }) async {
    // ignore: prefer_const_declarations
    final path = r'/domains/available-domains';
  ...
  Future<DomainGroupsDto?> getAvailableDomains({ String? inboxType, }) async {
    final response = await getAvailableDomainsWithHttpInfo( inboxType: inboxType, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'DomainGroupsDto',) as DomainGroupsDto;
  ...
  ///
  /// * [String] inboxType:
  Future<Response> getMailSlurpDomainsWithHttpInfo({ String? inboxType, }) async {
    // ignore: prefer_const_declarations
    final path = r'/domains/mailslurp-domains';
  ...
  Future<DomainGroupsDto?> getMailSlurpDomains({ String? inboxType, }) async {
    final response = await getMailSlurpDomainsWithHttpInfo( inboxType: inboxType, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
