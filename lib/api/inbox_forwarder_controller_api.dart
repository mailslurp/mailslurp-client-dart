...
class InboxForwarderControllerApi {
  ...
  Future<Response> getAllInboxForwarderEventsWithHttpInfo({ int? page, int? size, String? inboxId, String? emailId, String? sentId, String? sort, }) async {
    // ignore: prefer_const_declarations
  ...
  /// * [String] sort:
  ///   Optional createdAt sort direction ASC or DESC
  Future<PageInboxForwarderEvents?> getAllInboxForwarderEvents({ int? page, int? size, String? inboxId, String? emailId, String? sentId, String? sort, }) async {
    final response = await getAllInboxForwarderEventsWithHttpInfo( page: page, size: size, inboxId: inboxId, emailId: emailId, sentId: sentId, sort: sort, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
