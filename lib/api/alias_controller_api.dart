...
class AliasControllerApi {
  ...
  Future<Response> getAliasesWithHttpInfo({ String? search, int? page, int? size, String? sort, DateTime? since, DateTime? before, }) async {
    // ignore: prefer_const_declarations
    final path = r'/aliases';
  ...
  Future<PageAlias?> getAliases({ String? search, int? page, int? size, String? sort, DateTime? since, DateTime? before, }) async {
    final response = await getAliasesWithHttpInfo( search: search, page: page, size: size, sort: sort, since: since, before: before, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> getThreadWithHttpInfo(String threadId,) async {
    // ignore: prefer_const_declarations
    final path = r'/aliases/threads/{threadId}'
  ...
  ///
  /// * [String] threadId (required):
  Future<ThreadProjection?> getThread(String threadId,) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ThreadProjection',) as ThreadProjection;
  ...
  /// * [DateTime] before:
  ///   Optional filter by sent before given date time
  Future<Response> getThreadsPaginatedWithHttpInfo({ int? page, int? size, String? sort, DateTime? since, DateTime? before, }) async {
    // ignore: prefer_const_declarations
    final path = r'/aliases/threads';
  ...
  /// * [DateTime] before:
  ///   Optional filter by sent before given date time
  Future<PageThreadProjection?> getThreadsPaginated({ int? page, int? size, String? sort, DateTime? since, DateTime? before, }) async {
    final response = await getThreadsPaginatedWithHttpInfo( page: page, size: size, sort: sort, since: since, before: before, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
