...
class AliasControllerApi {
  ...
  /// Get validation result from alias verification with HTTP info returned
  ///
  /// Verify an email alias email address with the verification token that was emailed to the address
  Future<Response> getAliasVerificationWithHttpInfo(String aliasId, String emailAddress, String verificationToken) async {
    Object postBody;
    ...
    if(emailAddress == null) {
     throw ApiException(400, "Missing required param: emailAddress");
    }
    if(verificationToken == null) {
     throw ApiException(400, "Missing required param: verificationToken");
    }

    // create path and map variables
    String path = "/aliases/{aliasId}/verification".replaceAll("{format}","json").replaceAll("{" + "aliasId" + "}", aliasId.toString());
    ...
    Map<String, String> formParams = {};
      ...
      queryParams.addAll(_convertParametersForCollectionFormat("", "verificationToken", verificationToken));
  ...
  Future<AliasVerificationResult> getAliasVerification(String aliasId, String emailAddress, String verificationToken) async {
    Response response = await getAliasVerificationWithHttpInfo(aliasId, emailAddress, verificationToken);
    if(response.statusCode >= 400) {
      throw ApiException(response.statusCode, _decodeBodyBytes(response));
    } else if(response.body != null) {
      return apiClient.deserialize(_decodeBodyBytes(response), 'AliasVerificationResult') as AliasVerificationResult;
    } else {
      return null;
    }
  }
