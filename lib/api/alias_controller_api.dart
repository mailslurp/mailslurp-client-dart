...
class AliasControllerApi {
  ...
  Future<PageAliasThreadProjection?> getAliasThreads(String aliasId, { int? page, int? size, String? sort, DateTime? since, DateTime? before, }) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'PageAliasThreadProjection',) as PageAliasThreadProjection;
  ...
  Future<AliasThreadProjection?> getThread(String threadId,) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AliasThreadProjection',) as AliasThreadProjection;
